...
function checkStatus(color) {
  if (game.in_checkmate()) {
    $('#status').html(`<b>Echec et mat!</b> Oops, <b>${color}</b> perdu.`);
  } else if (game.insufficient_material()) {
    $('#status').html(`<b>Match nul!!</b> (Insufficient Material)`);
  } else if (game.in_threefold_repetition()) {
    $('#status').html(`<b>Match nul!!</b> (Threefold Repetition)`);
  } else if (game.in_stalemate()) {
    $('#status').html(`<b>Match nul!!</b> (Stalemate)`);
  } else if (game.in_draw()) {
    $('#status').html(`<b>Match nul!!</b> (50-move Rule)`);
  } else if (game.in_check()) {
    $('#status').html(`Oops, <b>${color}</b> est en <b>echec!</b>`);
    return false;
  } else {
    $('#status').html(`Pas d'echec, echec et mat, ou match nul`);
    return false;
  }
  return true;
}
...
function updateAdvantage() {
  if (globalSum > 0) {
    $('#advantageColor').text('Noir');
    $('#advantageNumber').text(globalSum);
  } else if (globalSum < 0) {
    $('#advantageColor').text('Blanc');
    $('#advantageNumber').text(-globalSum);
  } else {
    $('#advantageColor').text('Aucun des deux');
    $('#advantageNumber').text(globalSum);
  }
  $('#advantageBar').attr({
